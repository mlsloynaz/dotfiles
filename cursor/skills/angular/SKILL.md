...
description: Enforces TypeScript and Angular best practices per the Angular Developer's Guide. Use when writing or reviewing TypeScript, Angular components, templates, services, state management, or when the user asks for Angular/TypeScript guidance.
---

# Angular & TypeScript Best Practices

**Angular Developer's Guide** — Author: Ernesto Bonet\<ernesto.bonet@bydesign.com\>. For questions on standards, conventions, or architecture, reach out to him (acting lead Angular developer).
...
Our Angular project lives in **SOA/Applications/Web/AngularApp** and has been a mix of conventions. This guide defines a single standard that all Angular developers should follow. The goal is to eventually clean up existing code to match these conventions.

### Setup

A new Angular project setup addresses long-standing package configuration issues, enforces the guide’s code style via an updated **lint configuration**, and reorganizes framework code into **lib** and **core** projects.

- **If your branch is off `develop`:** These changes are already included.
- **If your branch is not off `develop` or predates this setup:** Cherry-pick the changes from **`bbelna/standards_eslintonly`**, delete **package-lock.json** (if present) and **node_modules**, then run **`npm i`** in the project root.

---

## Overview

This skill reflects conventions and standards from our **Angular Developer's Guide**. The guide follows industry standards (for Angular versions before v20), with a few deliberate differences:

- **Company-specific:** e.g. `_index` instead of `index`; `$` for observables and subjects.
- **Location:** The guide lives under **Programming Standards** (moved there 2025-08-26).

When working in Angular apps, follow this skill and the full guide. Below is a quick reference; the changelog tracks what’s been added to the guide over time.

**Alignment:** The same standards are reflected in [.github/copilot-instructions.md](/.github/copilot-instructions.md) (GitHub Copilot). That file includes an Angular 14–style component example (BehaviorSubject + async pipe), resource links, and detailed style rules.

## Guide contents (quick reference)

This summarizes the developer guide for quick reference. See the full guide for details.

**Projects** — Framework: Core (core), Utilities/Common (utils), Library (lib), Loqate SDK (loqate-sdk). Features: Address (address), Autoship (autoship), Orders (orders), Party (party), Payment (payment), Promotion (promotion), Geography (geography). Applications: Checkout 2.0 / My Account 2.0 (shop), Revolution (revolution).

**Project Structure** — File & directory names (casing, file suffixing, file names); root directory structure; colocated files; structuring component files & directories; library names; barrel files (overview, why).

**Code Style** — General: Tabs, Typing, Accessibility modifiers, Line length, Commas, Braces, Arrays, Injections, Property initialization, TSDoc, End of file. Names: Casing, Self-documenting. Interfaces, Enums. Properties: Ordering, Read-only. RxJS. Exports: Naming, One-export-per-file, Utility methods. Imports: Aliased, Relative, Simplified alias/relative rules, Circular imports.

**Tests & Mocks** — Overview; Guidelines; Colocated; Mocks; Unit testing for new code.

**Modules & Standalones** — Overview; Guidelines; Standalones; Modules; Ordering; Avoid provider functions in standalones.

**Components** — Introduction; Guidelines; Directory & suffix; Extends base; Selectors; Standalone; Change detection; Use resolvers.

**Injectables** — Introduction; Guidelines; No singletons without justification; Self-documenting; Separation of concerns. Types: APIs, Bridges, Stores, Initializers, Storages, Loaders, Services, Extensions.

**Stores** — Introduction; Store; Mixins (Defaultable, Destroyable, Mutable, Settable); Architecture; Directory & suffix; Injections; Store chains; Getters; Set by.

**Helpers** — Introduction; Guidelines; Directory & suffix; Non-constructable & non-injectable; Naming; Methods.

**Mappers** — Introduction; Guidelines; Directory & suffix; Non-constructable & non-injectable; Naming; Methods.

**Resolvers** — Overview; Guidelines; Directory & suffix; Use stores.

**Providers** — Overview; Guidelines; Directory & suffix; One-export-per-file; Separation; Naming.

**Operators** — Overview; Guidelines; Directory & suffix; One-export-per-file; Selectors.

**Custom Source Solutions** — Overview; Guidelines; Example; Extensions; SS CS stubs; Directory & structure; Overrides.

## Projects

**Reference:** By Arianna Capasso. This section details each project in the Angular codebase (**SOA/Applications/Web/AngularApp**).

### Framework

Projects that provide application framework functionality.

| Project | Folder | Description |
|---------|--------|-------------|
| **Core** | core | Core application functionality: HTTP interceptor, API client, configuration service, etc. Much of this currently lives in utils but will move here. Code must be core application functionality only—no feature-specific code. |
| **Utilities / Common** | utils | Common/shared folder; will be renamed to **common** (since "utils" in projects holds non-specific TS code). Application-specific reusable functionality that is not feature-bound: base components, common components (e.g. message component), models that don’t belong in a feature project. |
| **Library** | lib | Non-application-specific reusable TypeScript. Not core app functionality (unlike core) and not application-specific (unlike common). Examples: store library, types like Maybe and Nullable. |
| **Loqate SDK** | loqate-sdk | Exposes Loqate for address lookup and verification in feature and application projects. See [Loqate Address Verification](https://www.loqate.com/). |

### Features

Projects that provide reusable feature functionality.

| Project | Folder | Description |
|---------|--------|-------------|
| **Address** | address | Address models, services, and components (editing, displaying, etc.). |
| **Autoship** | autoship | Autoship models, injectables, and components (managing autoship order, creating autoship order, etc.). |
| **Geography** | geography | Country and territory models and their injectables. |
| **Orders** | orders | Orders functionality. Currently slim; much of the shop application should be refactored into here. |
| **Party** | party | Party models, services, and components (party orders, party payments, party order payments, party rewards). Used by Revolution. |
| **Payment** | payment | Payment models, services, and components for performing payment. |
| **Promotion** | promotion | Promotions models, services, and components for displaying promotions. |
...
| Project | Folder | Description |
|---------|--------|-------------|
| **Checkout 2.0 / My Account 2.0** | shop | Checkout 2.0 and My Account 2.0. Future: refactor into separate **checkout** and **my-account** application projects. Relative to `shop/src/app`: Checkout 2.0 is in **checkout**; My Account 2.0 is in **shop/customer**. |
| **Revolution** | revolution | Angular implementations of Revolution. Revolution is primarily .NET MVC; newer parts are Angular and live here. |

## Project structure

Guidelines for structuring directories, naming, and barrel files. When in doubt about how to structure something, reach out to Brandon Belna.

### Avoiding redundant names

- **Inside a folder, don’t repeat the folder name in the component/dir name.** Example: inside an **address** folder, use **"display"** (and e.g. `display.component.ts`), not **"address-display"**.
- **Inside a library/project, don’t repeat the project name.** In the **address** library, do not create a subfolder named "address" or "addresses", or prefix component folders with "address". The project name already provides that context. Example in the address library:
  - **edit** → `address-edit.component.ts`
  - **address** (root component) → `address.component.ts`
- **Components that match the project or directory name** can live in the **root** of that directory. If a component’s name matches the directory (e.g. party order in the party order folder), it can sit in that directory.

### Library names

- Follow **singular/plural** convention: if the library handles a single concept, use **singular**; if it handles a plural concept (and possibly singular too), use **plural**.
- Currently all libraries are singular; projects will be renamed to follow this convention over time.

### Barrel files

See also **Imports** under Code Style.

**Overview**

- Barrel files enable clean imports (e.g. `import { X, Y } from '@bydesign/address'`).
- Use a barrel in **every directory that contains code**. Name it **`_index`** (not `index`).
- We use **`_index`** so it sorts to the top and is clearly separate from feature code. When re-exporting a directory you must use `export * from './dir/_index'` instead of `export * from './dir'`; with TS path configs everything else behaves like a normal index.

**Contents and order**

- Export **everything** in that directory.
- Order: **by type** (subdirectories first, then files), then **alphabetically**.
- If the directory has subdirectories, export each subdirectory’s **`_index.ts`** first (directory exports at the top), then a newline, then file exports.
- This pattern continues up to the **root** `_index.ts`: in **libraries** the root is **`src/lib`**; in **apps** it’s **`src/app`**.
- **`public-api.ts`** should export only the root `_index.ts`.

**Example (address library, root = src/lib)**

- `_index.ts` exports: `./components/_index`, `./models/_index`, `./services/_index`, … then `./address.module`.
- `./components/_index` exports: `./address-edit/_index`, `./custom-fields/_index`, `./geographical-drilldown/_index`, then `address-edit.component`, etc.
- `./models/_index` exports: `addressConfigurationType`, `addressDropdown`, etc.
- `./services/_index` exports: `./interfaces/_index`, `address.service`, etc.

**Why barrel files?**

- Industry standard (npm packages, Nx Angular). We use **named imports** (`import { X } from …`); tree shaking only pulls what’s used. Angular architecture avoids side effects from importing a file, so barrel files do not hurt bundle size or build time.
- Not seeing the exact file path in the import is acceptable: we use “go to implementation” rather than the path to find code, and aliased imports improve readability. Circular imports are the main risk—avoid them by following the **aliased vs relative import rules** in Code Style. The readability gain outweighs the small maintenance cost of keeping barrel files updated.

## Code style

**Reference:** By Arianna Capasso. This section details code styling standards. Most of it is enforced by the lint file (e.g. from `bbelna/standards` or a branch that has it); ESLint also enforces spacing, newlines between blocks, bracket consistency, etc.

### General

- **Tabs:** 2 spaces.
- **Typing:** Properties, function parameters, and return types must be typed. Locals and arrow function parameters need not be typed unless you are defining an arrow function.
- **Types:** Use **enums** instead of primitive unions (`'credit_card' | 'direct_deposit'` or `1 | 2`). Use **Nullable\<T\>** for `T | null`; **Maybe\<T\>** for optional props/vars (`thing?: Maybe<Thing>`); **MaybePromise\<T\>** for `T | Promise<T>`.
- **Accessibility:** Use `private` and `protected`. **Never use `public`** (default). Exception: constructor parameters that must be public.
- **Line length:** Max **80 characters** for all file types (HTML, SCSS, TS). If a line exceeds 80 chars, refactor (e.g. break into functions, put arguments on separate lines). For long function invocations, put parameters on their own lines. Chained calls can continue on new lines. **Exception:** long import paths may exceed 80 chars when necessary (prefer breaking imports onto separate lines).
- **Commas:** Use a **dangling comma** when list elements are on their own lines.
- **Braces:** Same line (with exceptions). If function arguments are on separate lines and one argument is an object, put the opening brace on the same line as the call. Empty braces: `{}`. Single-line content: space before and after, e.g. `() => { this.thing = true; }`.
- **Arrays:** No space inside brackets: `[1, 2, 3]`, `[]`. Long or complex arrays: one item per line with dangling comma.
- **Injections:** Use **`readonly`** and **`inject()`**; do not inject via constructor. This allows extending classes without re-declaring parent injections and reduces constructor use to real initialization.
- **Property initialization:** Initialize in the property declaration when possible. Exception: when a public observable is derived from a private subject, initialize the subject in the declaration and the observable in the constructor.
- **TSDoc:** Required for almost everything. **Exception:** injection properties do not need TSDoc. Document params, return, generics. **Tag order:** `@template`, then `@param`, then `@returns`. Multi-line tag text and list items: align to the start of the text after the tag.
- **End of file:** Every code file must end with a newline.
- **Conditionals:** When all paths return, use **if-else** (not a bare `if` with return and the “else” logic outside an else block).

### Names

- **Casing:** camelCase for variables/properties and functions. PascalCase for types (classes, enums, interfaces). API model properties may be PascalCase from the API. Abbreviations: capitalize only to follow convention (e.g. `httpTest`, `isHttp`, `HttpInterceptor`).
- **Self-documenting:** Variables, methods, type-likes, and generics should have self-documenting names.
- **Interfaces:** Do **not** prefix with **I**, except when you must distinguish a contract from its implementation (e.g. `IAddressExtensionsService` for client overrides). Avoid **I** “just because it’s an interface.”
- **Enums:** Do not suffix with `Enum`. Use singular names (e.g. `PaymentType`).

### Constructors & ngOnInit

- Do **not** use `ngOnInit` for initialization unless the logic **depends on input values**. Inputs are not set until `ngOnInit`. All other initialization belongs in the **constructor**.

### Properties

**Ordering** (newlines between groups):

1. ViewChilds  
2. Inputs  
3. Outputs  
4. Public (excluding injections) → public getters and setters  
5. Protected (excluding injections) → protected getters and setters  
6. Private (excluding injections) → private getters and setters  
7. Public injections  
8. Protected injections  
9. Private injections  

For getters/setters: **getter first**, then setter; separate with newlines above and below.

**Read-only:** For a public read-only property, use a **private** property prefixed with **`_`** and a **public getter** that returns it. Use **`_`** only when there is a corresponding public property (the public name has no prefix). For **object types**, return a **clone** (e.g. `ObjectHelper.clone`) so callers cannot mutate.

**RxJS:** Suffix Observable/Subject variables and methods with **`$`**. Pattern: public `user$` (observable), private `_user$` (subject), private `_user` (data).

### Exports

**Naming:** Export names should be suffixed with the **singular** form of the root directory (e.g. under `apis` → suffix `Api`, e.g. `OnlineOrderApi`). **Exceptions (no suffix, or special):** constants, enums, **models**, **types**, **utils**. **Extensions** are suffixed with **"Extensions"** (plural). For utils there is no file suffix (no `.util`). This applies to the **exported member name**, not necessarily the file name.

**One-export-per-file:** Default is one export per file. **Exceptions:** (1) Related, utility-focused exports in a **colocated** file or **mock** file — use `*.mock` or colocated `*.*.options` (e.g. `*.api.options`). (2) Colocated utility models for a component → `a.component.models.ts`; utility functions → `a.component.utils.ts`. Still separate by **type** (don’t mix models and utils in one file). When in doubt, ask or follow code review.

**Utility methods:** Do not export loose utility functions from a file. Put them in a **helper**, **mapper**, or **service**. **Exceptions:** test code, **provider methods**, and **operators** (providers and operators still follow one-export-per-file). General reusable TS utilities (e.g. `hot` for Observables) may be exported as **one-file-per-method** under **lib** `utils`.

### Imports

**Aliased imports (`@bydesign/<project>`)** when the imported file is:

1. **Outside current project** → `@bydesign/<project>`  
2. **Outside current root directory** (root = e.g. components, models, enums) → `@bydesign/<project>/<other-root>`, e.g. from `utils/src/lib/components` to `utils/src/lib/models` → `@bydesign/utils/models`  
3. **In a subdirectory of current directory** → `@bydesign/<project>/<path-to-subdir>`  
4. **In a sibling subdirectory** (same parent, different child) → same pattern, e.g. `utils/src/lib/components/another-component` from `some-component` → `@bydesign/utils/components/another-component`  

**Ordering:** Alphabetical. Imports with `@` or special chars before others; within a path, order by path segment then by symbol (camelCase then PascalCase, each alphabetically). Example: `import { apple, test, Test, Zebra } from '@a';`. If using both `@b` and `@b/...`, list `@b` first, then `@b/...` paths alphabetically.

**Relative imports** when the file is:

1. **In the same directory** as the current file, or  
2. **Directly above** in the same root (only `../` needed; if you need `../` and then another subdir, use an alias).

**Simplified rules:** Use **aliased** unless: (1) the file is in the **same directory**, or (2) the file can be reached using **only `../`** (e.g. `../../order.model` is OK; `../../online/online-order.model` should be aliased).

**Circular imports:** Never use an **alias that exports the current file**. Example: inside `projects/address/.../components/shipping-method`, use `@bydesign/address/models` and `@bydesign/address/services`, **not** `@bydesign/address`. Using `@bydesign/address` pulls in the root barrel, which exports components, which exports shipping-method → circular dependency. Following the aliased/relative rules avoids circular imports.

## Tests & mocks

**Reference:** By Arianna Capasso. Standards for tests and mocks. **We use Jest for unit testing.**

### Guidelines

- **Colocated:** Test files (**\*.spec.ts**) must be **colocated** with the code they test (same directory as the source file).
- **Mocks:** Live in the **"mocks"** folder with suffix **".mock"**. Prefer a single **feature-based** file with multiple exports (e.g. `party.mock.ts`, `party-order.mock.ts`). **Do not export mocks as constants**—that causes state to leak across test contexts. Use **mock factories** (functions that return a new instance) instead.
- **Unit testing for new code:** All new code must have unit tests. When changing existing code, add tests for the code you updated. Adding tests that cover the full file you edited is strongly encouraged.

## Modules & standalones

**Reference:** By Arianna Capasso. Standards for modules and standalones (standalones are standalone “modules” in their own right).

### Guidelines

- **Standalones:** Any class that would normally go in a module’s **declarations** (directives, components, pipes) should have **`standalone: true`**.
- **Modules:** We are moving away from modules. If a module is still used: **one module per project**; the module file lives in the **root source directory** (**`src/lib`** for libraries, **`src/app`** for apps) with suffix **`.module`**. The module name is the **project name** (e.g. **PartyModule** for the party project).
- **Ordering:** In a module or standalone, list options in this order: **declarations**, **imports**, **exports**, **providers**. Within each list, order **alphabetically by suffix**, then by name. Example: `OneComponent`, `AService`, `SecondComponent`, `APipe`, `AnAPI` → **AnAPI**, **OneComponent**, **SecondComponent**, **APipe**, **AService** (Api → Component → Component → Pipe → Service).
- **Avoid provider functions in standalones:** Do **not** use provider functions (the `providers` array with factory functions) in a standalone. Use them only in the **root application** or in a **module**.
- **Singleton injectables in standalones:** Injectables that must be singletons (e.g. **stores**) must **not** be provided in a standalone—otherwise state would not persist across components. Provide them in the **root app module** instead, and use **`inject()`** in standalones to get them (do not add them to the standalone’s `providers`). **Strict rule:** If a singleton injectable is not provided in the **root app module**, it is **not** available for injection in a standalone.

## Components (architecture)

**Reference:** By Arianna Capasso. Architectural standards for components.

### Guidelines

- **Directory & suffix:** Components live in the **"components"** folder of the project and use the **".component"** suffix.
- **Extends base:** Components should **extend BaseComponent** or a descendant (e.g. **ResponsiveComponent**).
- **Selectors:** Use the **`bd`** prefix, e.g. `bd-party-orders`, `bd-address`.
- **Standalone:** Components should be **standalone** (`standalone: true`). Import and provide everything the component needs (services, pipes, components, etc.). Injectables provided at the **root** (see Providers) do not need to be in the component’s `providers` array.
- **Change detection:** Use the **OnPush** strategy by default (best performance). Use something else only when there is a clear reason.
- **Use resolvers:** Do **not** load data inside the component when you can avoid it; load data via a **resolver** (see Resolvers). **Exception:** For a page that loads from many endpoints and should **render as data arrives**, loading in the component is acceptable; still use a resolver for **"core"** data. Example: a page that shows an online order and its details—use a resolver for the **online order**, and load **details** in the component so the page can show the order first and fill in details dynamically.

## Injectables (architecture)

**Reference:** By Arianna Capasso. Architectural standards for injectables.

### Introduction

Many existing services are too broad and do too much—often a singleton that mixes an **API** and a **store** (e.g. AddressService, OnlineOrderService). Going forward:

- **Split by concern:** Use **two** injectables: an **API** (methods for the API) and a **Store** (state management). Do not combine both in one injectable.
- **Prefer specific types over "service":** Use **Api**, **Store**, **Bridge**, **Adapter**, etc. Only use "service" when the injectable doesn’t fit a clearer type.
- **No singletons by default:** Do **not** use `providedIn: 'root'` without a clear reason. Prefer providing injectables per module/component when possible.

### Guidelines

- **No singletons without justification:** Do not make an injectable a singleton unless justified.
- **Self-documenting:** Before naming something a service and putting it in services, check if it fits a more specific type (API, store, bridge, adapter, loader, etc.). **Bridges** and **adapters** follow design-pattern meanings—name after the pattern (e.g. StorageBridge) and use a matching folder (e.g. bridges). See [Refactoring Guru](https://refactoring.guru) or consult an AI when unsure.
- **Separation of concerns:** One concern per injectable. Do not have one injectable act as both an API and a store.

### Types

| Type | Folder | Suffix | Description |
|------|--------|--------|-------------|
| **APIs** | apis | .api | Expose methods for talking to the API only. Extend **Api** (under core; currently utils). **Pluralized** subject and filename (e.g. OnlineOrdersApi, online-orders.api). Use base Api’s `apiUrl` and `http`; soon an API client will replace this. |
| **Bridges** | bridges | .bridge | Abstract implementation and expose an implementation-agnostic interface (see Refactoring Guru). Example: StorageBridge for web (local storage) vs mobile (Capacitor storage), wired by compilation context. |
| **Stores** | stores | .store | Persist and manage state; get/update data via APIs. See Stores page. |
| **Initializers** | initializers | .initializer | Run at app startup to initialize something. (Implementation coming with checkout refactor.) |
| **Storages** | storages | .storage | Expose persistent storage (e.g. local storage, session storage). |
| **Resolvers** | resolvers | .resolver | Load data before a route/component is shown; use with stores (e.g. OnlineOrderResolver loads into OnlineOrderStore). See Resolvers page. |
| **Extensions** | extensions | .extensions | For **CS (client-specific)** code. SS provides a stub (e.g. PaymentExtensions with `paymentCompleted` default); CS overrides under clients folder. |
| **Services** | services | .service | Fallback when the injectable doesn’t fit another type. Prefer a more specific type when possible. |

For all types: use the folder and suffix above **unless colocated** (then follow colocation rules).

## Stores (architecture)

**Reference:** By Arianna Capasso. Custom store solution (we do not use NgRx).

### Introduction

A **store** holds data that persists across parts of the app and exposes ways to manage it via its **state** (e.g. UserStore holding a User). Stores are Angular injectables (`@Injectable`). Use stores wherever you need this behavior instead of a service that acts like a store (e.g. refactor AddressService into ShippingAddressStore, BillingAddressStore, and AddressApi). Create a class that **extends the Store class**; optionally apply **mixins** for defaultable, destroyable, mutable, or settable behavior.

### Store class (base)

**Public properties:**  
- **state$** — Observable of a copy of the store’s state; emits current state on subscription if it exists.  
- **stateChanged$** — Like state$ but does **not** emit on subscription; only on updates.  
- **event$** — Emits **StateEvent** (Initialized, Updated, Destroyed).  
- **changedProperties$** — Emits the list of state properties that changed (useful to react to specific properties).  
- **exists$** — Emits whether state exists; emits on subscription.

**Public method:**  
- **get()** — Returns a copy of the current state.

**Protected methods (for subclasses):**  
- **withState(fn)** — Runs `fn(state)` if state exists; otherwise throws. Use to avoid null checks.  
- **update(state)** — Sets state to a full new model. Do not override.  
- **patch(stateChanges)** — Updates only given properties; state must already be initialized.  
- **sync(changedProperties)** — Called after state update; loops over changed properties and calls syncProperty, then afterSync. Do not override; override afterSync or syncProperty instead.  
- **afterSync(state)** — Override to run logic after sync (e.g. set other stores from this store’s state).  
- **syncProperty(property)** — Override to run logic when a specific property changes (e.g. persist to local storage).  
- **validateUpdate(state, event)** — Override to validate changes; return false to reject (update throws).

### Mixins

| Mixin | Adds |
|-------|------|
| **Defaultable** | **init(stateProperties?)** to initialize state; optional props → default state. Store must implement protected **construct()** returning the default state. |
| **Destroyable** | **destroy()** and **destroyed$** observable. |
| **Mutable** | **mutate(stateChanges)** — public wrapper for patch. |
| **Settable** | **set(state)** — public wrapper for update. |

### Guidelines

- **Directory & suffix:** **stores** folder, **.store** suffix, unless colocated (then follow colocation rules).
- **Injections:** Only inject **APIs** and **other stores**. APIs for backend calls; other stores for **store chains**. Other injections allowed with justification.
- **Store chains:** One store’s update can trigger updates in other stores (e.g. OnlineOrderStore sets PromotionsStore, ShippingAddressStore). Implement in **afterSync** or **syncProperty**. Dependent stores must **not** inject the store that sets them (avoids circular dependency). The **parent** store should set the **child** stores; children should not subscribe to the parent to set themselves. That way **sync** waits for child stores to be set before returning, so after an update you know all dependent stores are ready.
- **Getters:** Implement getters on the store that return state properties (e.g. `get apiUrl()` from AppSettingsStore state). Getters should **throw** if there is no state—use **withState**, e.g. `get property(): U { return this.withState((s) => s.property); }`. Do **not** implement setters; only set state through methods.
- **Set By:** If state depends on something else to be set, add a **setBy** method (e.g. **setByOnlineOrderId(onlineOrderId)** on ShippingAddressStore). Fits store chains: e.g. OnlineOrderStore calls `promotionsStore.setByOnlineOrderId(this.state.id)`.

## Helpers

**Reference:** By Arianna Capasso. Standards for helper classes.

### Introduction

**Helpers** are **non-constructable** and **non-injectable** classes that expose **static** utility methods for a given subject (e.g. **OrderHelper** for Order models). The subject can be a model or anything that needs utility functions.

### Guidelines

- **Directory & suffix:** **helpers** folder, **.helper** suffix, unless colocated (then follow colocation rules).
- **Non-injectable & non-constructable:** Helpers are not injected and must not be constructed. Use them as classes with static methods only.
- **Naming:** **\<SubjectName\>Helper** (e.g. OrderHelper, UserHelper).
- **Methods:** Only **static** methods that are related to the helper’s subject.

## Mappers

**Reference:** By Arianna Capasso. Standards for mapper classes.

### Introduction

**Mappers** are a kind of **helper** that expose only **static** methods for **mapping to a specific model**. They follow all helper rules plus the method rules below. Example: **OrderMapper** has static methods that take other models and return an **Order**.

### Guidelines

- **Directory & suffix:** **mappers** folder, **.mapper** suffix, unless colocated (then follow colocation rules).
- **Non-injectable & non-constructable:** Same as helpers—mappers are not injected and must not be constructed.
- **Naming:** **\<ModelName\>Mapper** (e.g. OrderMapper, AppSettingsMapper).
- **Methods:** Static methods that **return** the model the mapper is named after. Method names use the **from\*** pattern. Examples: **AppSettingsMapper.fromJson(appSettingsJson)** returns AppSettings; **AppSettingsJsonMapper.fromModel(appSettings)** returns AppSettingsJson.

## Resolvers

**Reference:** By Arianna Capasso. Resolvers and how they should be used.

### Introduction

**Resolvers** are injectables that run **before** a route’s component loads and load the data that component(s) need. They implement Angular’s **Resolve\<T\>** interface, where **T** is the type of data being loaded. See [Resolve • Angular](https://angular.dev/api/router/Resolve). Using resolvers keeps components focused on displaying and manipulating data instead of loading it. As noted in **Components**, use resolvers to load data; there is an exception for dynamic/progressive loading—see Components > Use Resolvers.

### Guidelines

- **Directory & suffix:** **resolvers** folder, **.resolver** suffix, unless colocated (then follow colocation rules).
- **Use stores:** Resolvers can put data on the **activated route’s data**. When that data should **persist across routes** (most of the time), also use **stores**: (1) Have the resolver **initialize the store** with the loaded data and **return** the data from the store. (2) The component then reads from the store. That way data can be updated across routes without saving to the backend every time. Without a store, the resolver would hit the API on every navigation. When implementing this: **(1)** If the data is **already in the store**, return it from the store. **(2)** If not, **load it into the store**, then return it from the store.

## Providers

**Reference:** By Arianna Capasso. Provider methods.

### Overview

**Provider methods** live in the **providers** folder and **return a list of injectables** to be provided (e.g. at app root or in a module). This is the standard pattern in modern Angular. Create a provider when you have a group of injectables that belong together—e.g. **provideAppStores** for core app stores, **providePartyStores** for the party feature’s stores. In a standalone-only world, **stores** are the main use (they must persist app-wide; providing them only in components makes them non-persistent). We still use modules in places, so we also have providers per injectable type (e.g. **providePartyApis**) for module-based components. Long term, **provide*Stores** will dominate; APIs and the like will be injected directly in standalones.

### Guidelines

- **Directory & suffix:** **providers** folder, **.provider** suffix, unless colocated (then follow colocation rules).
- **One-export-per-file:** One provider function per file, even though providers are utility-style.
- **Separation:** Separate provider functions **by injectable type** (e.g. **provide*Stores**, **provide*Services**), not a single provide*Injectables. Only add a provider for injectables that must be provided at **application root** or in a **module**. With modules we still need providers for other types so they can be provided.
- **Naming:** **provide\<Project\>\<InjectableTypePlural\>** (e.g. provideAppStores, provideOrdersApis, provideCheckoutServices).

## Operators

**Reference:** By Arianna Capasso. Custom RxJS operators (“operators”).

### Overview

**Operators** are custom RxJS operators we use to simplify pipes. When pipe logic is reusable, turn it into an operator and use it in streams instead of duplicating logic.

### Guidelines

- **Directory & suffix:** **operators** folder, **.operator** suffix, unless colocated (then follow colocation rules).
- **One-export-per-file:** One operator per file.
- **Selectors:** For any parameter that is **not** from injections, use a **selector function** of type **(data: T) => \<param type\>**. That way the value can be taken from the **data stream**. If the value is not from the stream, the selector can still be used (e.g. `() => this.orderId`). Exposing parameters as selectors keeps the option to take them from the stream when needed.

## Custom source solutions

**Reference:** By Arianna Capasso. How to implement custom source (CS) solutions.

### Overview

**CS (custom source)** = client-specific code that is built only for that client (e.g. HealthyHome: government name/ID on address edit, China-specific validation). **SS (single source)** = code in the main codebase, built and deployed for all clients.

**When to use CS vs feature-flag SS:** Use **CS** when the feature is clearly for a single client and would never be used by others. Use **feature-flag-based SS** when the feature could apply to other clients. The card usually specifies CS or SS; if you think the designation is wrong, discuss with the developers and card owner.

The rest of this section describes how to add CS code to the Angular codebase.

### Guidelines

- **Example:** A full CS example exists under **HealthyHome** in the **clients** folder; use it as reference.
- **Extensions:** A special injectable type for **service-layer** CS logic. They live under an **extensions** directory, with filenames suffixed **.extensions** and type names suffixed **Extensions**. See SS CS Stubs for an example (older code may use names like AddressExtensionsService; under current standards it would be AddressExtensions).
- **SS CS stubs:** To expose CS from SS, add **stubs** in the SS code. Keep stubs in **their own file** (e.g. **AddressExtensions**), not inside the main service (e.g. AddressService). The main service injects the extensions and calls the CS methods where needed. The SS file contains stub implementations; CS overrides them.
- **Directory & structure:** SS stubs live in the **main codebase** like any other code. CS overrides live under **clients/\<client\>/projects**. The structure under **clients/\<client\>/projects** **mirrors** the root **projects** structure. Example—override **AddressEditCustomFieldsComponent** for HealthyHome:  
  - **SS:** `projects/address/src/lib/components/address-edit/address-edit-custom-fields/address-edit-custom-fields.component.ts`  
  - **CS:** `clients/healthyhome/projects/address/src/lib/components/address-edit/address-edit-custom-fields/address-edit-custom-fields.component.ts`
- **Overrides:** In **angular.json**, under **configurations** for the application that needs the override, set **fileReplacements**: **replace** = path to the SS file, **with** = path to the CS file. The Angular compiler then uses the CS file instead of the SS file for that build.

## Changelog (guide updates)

| Date | Author | Summary |
|------|--------|--------|
| **v2025-09-11** | Brandon Belna | Injectables: added Extensions. Project Structure & Code Style: extensions (no singularization; `.extensions` suffix; names suffixed "Extensions"). Added Custom Source Solutions. Renamed Tests → Tests & Mocks. |
| **v2025-09-02** | Brandon Belna | Code Style: Maybe\<T\>, Nullable\<T\>, MaybePromise\<T\> for optional/nullable and T \| Promise\<T\>. Line length applies to HTML, SCSS, TS. Properties: clarified getter/setter ordering. |
| **v2025-08-27** | Brandon Belna | Stores: new `withState`. Resolvers: exception when page is dynamically rendered (core data resolvers still recommended). Standalones: Singleton Injectables. Code Style: Constructors & ngOnInit. |
| **v2025-08-26** | Brandon Belna | Guide moved to Programming Standards. Imports: aliased vs relative rules, alphabetical order, Simplified Alias/Relative Rules. TS Doc: alignment and tag order. Projects: Geography (geography). Root: operators folder. Providers: one per injectable type; future focus on stores. One-Export-per-File. Colocation: "unless colocated (follow colocation rules)." Added Modules & Standalones, Operators. |
| **v2025-08-25** | Brandon Belna | Imports: when to use aliased vs relative. TS Doc: injections don’t require TS doc. Helpers/Mappers: combined Non-Injectable & Non-Constructable. Colocated: spec/test files with code. Components: Use Resolvers. Stores: getters throw if state missing; `afterSync(state)`; Tests, TOC, Resolvers, Providers pages. |
...
- Always include type annotations (except for arrow functions)
- Use enums instead of union types for fixed values
- Always add TSDoc (except for injection properties); align text and follow tag ordering
- Use **Maybe\<T\>** and **Nullable\<T\>** for optional/nullable properties and variables
- Use **MaybePromise\<T\>** for functions that return `T | Promise<T>`
...
- Access modifiers: never use `public` (it’s default); use `private` and `protected` appropriately
- Keep change detection optimized
...
- Use resolvers for "core" data loading
- Initialize in constructor when possible; use `ngOnInit` only when necessary
- Keep component state immutable

**Property order** (newlines between groups): 1) ViewChilds 2) Inputs 3) Outputs 4) Public (excluding injections) 5) Protected (excluding injections) 6) Private (excluding injections) 7) Public injections 8) Protected injections 9) Private injections.
...
- When using RxJS: expose state as Observable (e.g. `.asObservable()`), name observables with **$** suffix; use Store base class where applicable
...
- Use **trackBy** with `@for` / `*ngFor` for better performance
- Use proper typing for template variables; break complex templates into smaller components
...
- Document public API methods with TSDoc; handle errors appropriately; use TypeScript interfaces for responses

**Extensions:** Do not follow singularization; filenames suffixed with `.extensions`, class names suffixed with `Extensions`.
...
- **Aliased imports (`@bydesign/*`)** when importing from: outside current project, outside current root directory, subdirectories, or sibling subdirectories.
- **Relative imports** only for: files in the same directory, or files directly above in the same root directory.

## General style

- 2 spaces for indentation; maximum line length **80 characters** (applies to HTML, SCSS, and TS)
- Newline at end of every file; dangling commas for multi-line lists
- Braces on same line (with noted exceptions); use if-else when all paths return
- Property initialization in declaration when possible

## Store patterns

- Use Store base class for state management
- Getters should throw when state doesn’t exist
- Use **withState** for state updates and **afterSync(state)** for post-sync operations
- Prefer stores over singleton services in standalone components

## Example (RxJS / BehaviorSubject pattern)

When using BehaviorSubject/Observable instead of signals (e.g. Angular 14 style or existing code):
...
export class ExampleComponent {
  private readonly _isServerRunning = new BehaviorSubject<boolean>(true);
  readonly isServerRunning$ = this._isServerRunning.asObservable();

  toggleServerStatus(): void {
    this._isServerRunning.next(!this._isServerRunning.value);
  }
}
```

Template: use `*ngIf="isServerRunning$ | async as isServerRunning"` (or `@if` with async). File convention: `name.component.{ts|html,css}`; logic in .ts, styles in .css, template in .html.

## Reference

- **Full guide:** Angular Developer's Guide (Programming Standards). For Stores, Resolvers, Providers, Operators, Modules & Standalones, Extensions, Tests & Mocks, Code Style (imports, TS Doc, line length, properties, constructors/ngOnInit), and Custom Source Solutions, see the guide.
- **Copilot:** [.github/copilot-instructions.md](/.github/copilot-instructions.md) — component example (BehaviorSubject + async pipe), resources, and detailed style rules.
- **Angular:** [angular.dev/essentials](https://angular.dev/essentials) (components, signals, templates, dependency-injection); [angular.dev/style-guide](https://angular.dev/style-guide).
- **Local doc:** [best-practices.md](best-practices.md)

## Additional context (llm-full)
